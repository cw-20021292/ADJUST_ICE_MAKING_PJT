...
1. [AutoIceMake_ProcessIceMaking() 함수 동작 원리](#1-AutoIceMake_ProcessIceMaking-함수-동작-원리)
...
## 1. AutoIceMake_ProcessIceMaking() 함수 동작 원리
...
| **🆕 STATE_42_NEXT_ICE_AMOUNT_CAL** | 42 | 다음 제빙시간 계산 | - **SetDrainPrevFlowHz()**<br>- **SetDrainCurFlowHz()**<br>- **AutoIceMake_ProcessIceMaking()** 호출 | → STATE_43 | **[기술과제 핵심]**<br>다음 제빙시간 보정 |
...
case STATE_42_NEXT_ICE_AMOUNT_CAL:
    ...
    AutoIceMake_ProcessIceMaking();                          // 다음 제빙시간 보정
...
3. **AutoIceMake_ProcessIceMaking() 호출**:
   - FIFO 평균 계산
   - ZONE 판정
   - 이론 비율 계산
   - **NextIceMakeTime 결정**
...
1. **`AutoIceMake_ProcessIceMaking()`** (work_ice_make.c)
   - 평균 계산, ZONE 판정, 비율 계산
   - NextIceMakeTime 결정
