...
```c
// ZONE 1: 심각 과제빙
if(IceAdjust.i16IceMakeAvgFlow > GetCCToHz(120))  // > 216Hz
{
    SetGain(ZONE_1_GAIN);  // 0.7
}
// ZONE 2: 살짝 과제빙
else if(IceAdjust.i16IceMakeAvgFlow > GetCCToHz(75))  // > 135Hz
{
    SetGain(ZONE_2_GAIN);  // 0.2
}
// ZONE 4: 심각 부족
else if(IceAdjust.i16IceMakeAvgFlow < GetCCToHz(35))  // < 63Hz
{
    SetGain(ZONE_4_GAIN);  // 0.7
}
// ZONE 3: 살짝 부족
else if(IceAdjust.i16IceMakeAvgFlow < GetCCToHz(50))  // < 90Hz
{
...
case STATE_41_DRAIN_FLOW_CALCUATE:
    // DrainFlowInput() 인터럽트로 i16IceMakeAfterFlowHz 증가
    if(u8DrainWaterLevel == Bit0_Drain_Water_Empty)
    {
...
**측정값:**
- `DrainFlow.i16IceMakeAfterFlowHz`: 제빙 후 배수된 물량 (Hz 단위)
- 유량센서 인터럽트로 자동 증가
