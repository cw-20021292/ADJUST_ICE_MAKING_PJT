...
**항상 목표 제빙량(예: 60~70ml)을 달성하도록 제빙 시간을 자동 조정하는 펌웨어**입니다.
...
```c
typedef struct {
    F32 f32Gain;                        // 현재 Zone의 보정 Gain
    U8  u8Cycle;                        // 제빙 사이클 카운터
    F32 f32Target;                      // 목표 제빙량 (Hz 단위)
    F32 f32Ratio;                       // 계산된 보정 비율
    U16 u16NextIceMakeTime;             // 다음 제빙 시간
    U16 u16ThisTimeIceMakeTime;         // 현재 제빙 시간
    I16 i16IceMakeAvgFlow;              // 평균 제빙 유량
    I16 i16IceMakeFlowHistory[3];       // 최근 3회 제빙 유량 히스토리
} ICE_ADJUST_T;
```
...
### Issue #1: 1~2회차 평균 계산 오류

**현재 상태**: 2회차에서 (1회 + 2회) / 2가 아니라 2회차 값만 /2 하는 버그 존재

**영향**: 초기 2회차 동안 Zone 판정이 부정확하며, Gain 조정이 잘못 적용될 수 있음

**해결 방안**: 히스토리 배열을 처음부터 사용하여 평균 계산 로직 개선 필요
...
**Expertise**: Embedded Firmware / BLDC / 냉각시스템 제어 로직 설계
